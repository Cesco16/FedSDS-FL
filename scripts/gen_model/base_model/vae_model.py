...
class VariationalAutoencoder(torch.nn.Module):
    ...
    def get_latent(self, input_data):
        input_data = np.array(input_data)
        input_data = torch.from_numpy(input_data).to(torch.device('cpu')).float()
        latent_output = self.Encoder(input_data)
        latent_params = self.latent_space.get_latent_params(latent_output)
        return [l.detach().cpu().numpy() for l in latent_params]
    ...
    def fit(self, data, train_params):
        ...
        optimizer = torch.optim.Adam(self.parameters(), lr=train_params['lr'], betas = (0.9,0.9)) #for MDS -> betas = (0.9,0.9)
