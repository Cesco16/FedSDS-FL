...
def preprocess_brca1(dataset_name):
    # Load data
    raw_df = pd.read_csv('./raw_data/sa_data/brca1/brca1.csv', index_col=0)
    ...
    label = raw_df[['event']]
    time = raw_df[['time']]
    raw_df = raw_df.drop(labels=['event', 'time'], axis=1)
    ...
    data_manager.norm_df, norm_params = data_manager.transform_data(df)
    #print(df)
    data_manager.imp_norm_df = data_manager.impute_data(data_manager.norm_df)
    #print(data_manager.imp_norm_df)
    # Create metadata for ctgan and tvae
    data_manager.get_metadata()

    return data_manager, norm_params
    
def preprocess_brca2(dataset_name):
    # Load data
    raw_df = pd.read_csv('./raw_data/sa_data/brca2/brca2.csv', index_col=0)
    raw_df = raw_df.sample(frac=1).reset_index(drop=True)  # Reorder the data randomly in case patients are ordered
...
def preprocess_lgg1(dataset_name):
    # Load data
    raw_df = pd.read_csv('./raw_data/sa_data/lgg1/lgg1.csv', index_col=0)
    raw_df = raw_df.sample(frac=1).reset_index(drop=True)  # Reorder the data randomly in case patients are ordered
...
def preprocess_lgg2(dataset_name):
    # Load data
    raw_df = pd.read_csv('./raw_data/sa_data/lgg2/lgg2.csv', index_col=0)
    raw_df = raw_df.sample(frac=1).reset_index(drop=True)  # Reorder the data randomly in case patients are ordered
...
def preprocess_mds1(dataset_name):
    #Load data
    raw_df = pd.read_csv('./raw_data/sa_data/mds1/mds1.csv')
    raw_df = raw_df.sample(frac=1).reset_index(drop=True)

    #Transform covariates and create df
    label = raw_df[['DeadOrAliveAtTransplantation']]
    time = raw_df[['OverallSurvival']]
    raw_df = raw_df.drop(labels=['DeadOrAliveAtTransplantation','OverallSurvival'], axis=1)
    raw_df = raw_df.drop(labels=['LeukemiaFreeSurvival','EvolutionToAcuteLeukemiaAtTransplantation','AMLOverallSurvival','DeadOrAMLAliveAtTransplantation','IPSSR','WHO 2016'],axis=1)
    survival_df = time.join(label)
    raw_df = raw_df.join(survival_df)
    raw_df = raw_df.rename(columns={raw_df.columns[-1]:'event', raw_df.columns[-2]:'time'})
    ...
    for i in range(df.shape[1]):
        ...
        if no_nan_values.size <= 2 and np.all(np.sort(no_nan_values).astype(int) ==
            ...
            #Check if values are floats but don't have decimals and transform to int. They are floats because of NaNs
            if no_nan_values.dtype == 'float64':
                no_nan_values.dtype = no_nan_values.astype(int)
            if np.unique(no_nan_values).size < 50 and np.amin(no_nan_values) == 0:
                feat_distributions.append(('categorical', (np.max(no_nan_values) + 1).astype(int)))
            else:
                feat_distributions.append(('gaussian', 2))
    ...
    data_manager.norm_df, norm_params = data_manager.transform_data(df)
    data_manager.imp_norm_df = data_manager.impute_data(data_manager.norm_df)
    ##data_manager.imp_norm_df = df ##data_manager.impute_data(data_manager.norm_df)
    #Create metadata for ctgan and tvae
    data_manager.get_metadata()

    return data_manager, norm_params

def preprocess_mds2(dataset_name):
    #Load data
    raw_df = pd.read_csv('./raw_data/sa_data/mds2/mds2.csv')
    raw_df = raw_df.sample(frac=1).reset_index(drop=True)
...
def preprocess_mds3(dataset_name):
    #Load data
    raw_df = pd.read_csv('./raw_data/sa_data/mds3/mds3.csv')
    raw_df = raw_df.sample(frac=1).reset_index(drop=True)
...
def preprocess_testmds1(dataset_name):
    #Load data
    raw_df = pd.read_csv('./raw_data/sa_data/mdstest1/mdstest1.csv', index_col=0)
    raw_df = raw_df.sample(frac=1).reset_index(drop=True)
...
def preprocess_testmds2(dataset_name):
    #Load data
    raw_df = pd.read_csv('./raw_data/sa_data/mdstest2/mdstest2.csv', index_col=0)
    raw_df = raw_df.sample(frac=1).reset_index(drop=True)
...
def preprocess_testmds3(dataset_name):
    #Load data
    raw_df = pd.read_csv('./raw_data/sa_data/mdstest3/mdstest3.csv', index_col=0)
    raw_df = raw_df.sample(frac=1).reset_index(drop=True)
...
def preprocess_by_name(dataset_name):
    ...
    elif dataset_name == 'mds1':
        return preprocess_mds1(dataset_name)
    elif dataset_name == 'mds2':
        return preprocess_mds2(dataset_name)
    elif dataset_name == 'mds3':
        return preprocess_mds3(dataset_name)
    elif dataset_name == 'testmds1':
        return preprocess_testmds1(dataset_name)
    elif dataset_name == 'testmds2':
        return preprocess_testmds2(dataset_name)
    elif dataset_name == 'testmds3':
        return preprocess_testmds3(dataset_name)
    elif dataset_name == 'brca1':
        return preprocess_brca1(dataset_name)
    elif dataset_name == 'brca2':
        return preprocess_brca2(dataset_name)
    elif dataset_name == 'lgg1':
        return preprocess_lgg1(dataset_name)
      elif dataset_name == 'lgg2':
        return preprocess_lgg2(dataset_name)
    else:
...
if __name__ == '__main__':
    ...
    for dataset_name in ['brca1','brca2']:#['testmds1','testmds2','testmds3','mds1','mds2','mds3']:#['mds1','mds2','mds3']:#['metabric', 'gbsg', 'whas', 'nwtco', 'pbc', 'std', 'mds']:
        data_manager, norm_params = preprocess_by_name(dataset_name)
        ...
        pd.DataFrame(norm_params).to_csv(save_path + 'norm_params.csv', index=False)
        #norm_params.to_csv(save_path + 'norm_params.csv', index=False)
